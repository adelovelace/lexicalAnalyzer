...
def show_parser(screen_input, label):
    INPUT = screen_input.get("1.0", "end-1c")
    text_result = validaRegla(INPUT)
    print(text_result)
    label["text"] = text_result

def both(input, lexico, parser):
    show_parser(input, parser)
...
def clear_all(input_text, table1, label):
...
parser_label.grid(row=4, column=0, sticky=tk.NSEW, columnspan=9, padx=(10, 10), pady=(0, 10))
...
tk.Button(middle_frame, text="Parser", command=lambda: show_parser(pantalla, parser_label)).grid(row=2)
tk.Button(middle_frame, text="Both", command=lambda: both(pantalla, lexico_table, parser_label)).grid(row=3)
tk.Button(middle_frame, text="Clear", command=lambda: clear_all(pantalla, lexico_table, parser_label)).grid(row=4)
